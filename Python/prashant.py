...
with open('D:/ddos/Main/logs/traffic_with_ddos_and_peak(2).json', 'r') as f:
...
kmeans = KMeans(n_clusters=3, n_init=10)  # 3 clusters: benign, hike, downward; 10 initialisadtions to choose the best from
...
#         j_test = update_j_test('C:/Users/isjsj/Downloads/Final-main/Final-main/SIH/logs/logs.json ')
...
#         cluster_label = kmeans.predict(scaled_test_features)  # Update to use scaled_test_features!
...
while True:
    try:
        ...
        # Predict the cluster label using the KMeans model
        cluster_label = kmeans.predict(scaled_test_features)
        ...
        if cluster_label == [0]:
            print("Cluster 0 detected. Starting the JS server...")

            # Check if the server is already running
            if node_server_process is None or node_server_process.poll() is not None:
                try:
                    # Start the Node.js server
                    node_server_process = subprocess.Popen(["node", "D:/ddos/SIH/tool.js"])
                    print("JavaScript server started.")
                except Exception as e:
                    print(f"Failed to start the JS server: {e}")
            else:
                print("Server is already running. Skipping restart.")
