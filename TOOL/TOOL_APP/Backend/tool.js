...
// Correct logDirectory path based on your clarified structure
const logDirectory = path.join(__dirname, "..", "..", "logs"); // This moves from backend to TOOL_APP to main, then into logs.
const logFilePath = path.join(logDirectory, "logs.json"); // Directly pointing to logs.json inside the logs folder.
const blockedIPsFilePath = path.join(logDirectory, "blocked_ips.json"); // Pointing to blocked_ips.json in logs folder.
...
const saveBlockedIPs = () => {
  try {
    ...
    fs.writeFileSync(blockedIPsFilePath, JSON.stringify(blockedData, null, 2));
    logger.info("Blocked IPs saved to JSON file.");
  } catch (error) {
    logger.error("Error saving blocked IPs to JSON file:", error);
  }
};
